...
def ensure_keys():
    ...
    if os.path.exists(SK_PATH) and os.path.exists(PK_PATH):
        ...
        pk_raw = sk.public_key().public_bytes(encoding=serialization.Encoding.Raw,
...
def pow_solve(kid16: bytes, c_eph_pub: bytes, window_id: int, salt: bytes, bits: int, limit_iters: int = 5_000_000) -> bytes:
    # cherche pow_nonce (8B) pour lequel sha256("PK6"|1|win|salt|kid|c_eph_pub|pow_nonce) a >= bits zéros initiaux
    for _ in range(limit_iters):
        ...
        if leading_zero_bits(h) >= bits:
            return nonce
    raise RuntimeError("PoW non trouvé (augmentez --pow-limit ou baissez la difficulté)")

# ---- SPA
def build_packet(server_pub_b64u: str, difficulty_bits: int, difficulty_salt_b64u: str, duration: int, totp_required: bool, sk_ed: ed25519.Ed25519PrivateKey, kid: str):
    s_pub = de_b64u(server_pub_b64u); diff_salt = de_b64u(difficulty_salt_b64u)
    kid_bytes = bytes.fromhex(kid)[:16]
    # clé éphémère X25519
    ...
    pow_nonce = pow_solve(kid_bytes, c_epub, win, diff_salt, difficulty_bits)
    # AEAD
    ...
    # payload signé
    payload = {"kid": kid, "ts": int(time.time()), "duration": int(duration), "inner_nonce": b64u(secrets.token_bytes(8))}
    if totp_required:
        code = read_totp_code()
        if code is None:
            print("TOTP requis : placez votre secret base32 dans", TOTP_PATH, file=sys.stderr)
            sys.exit(1)
    ...
    aad = b"PK6|" + struct.pack("!BIB", 1, win, difficulty_bits) + c_epub + kid_bytes + pow_nonce
    ct = aead.encrypt(aead_nonce, pt, aad)
    header = b"PK6" + bytes([1]) + struct.pack("!I", win) + bytes([difficulty_bits]) + c_epub + kid_bytes + pow_nonce + aead_nonce
    return header + ct
...
def main():
    ...
    code, _ = http_post_json(dst, args.http, "/enroll", {"kid": kid, "pubkey": b64u(pk_raw)})
    if code != 200:
        print(f"[WARN] /enroll HTTP {code} (peut être déjà enrôlé)", file=sys.stderr)

    # SPA
    pkt = build_packet(s_pub, diff_bits, diff_salt, args.duration, totp_required, sk, kid)
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s.sendto(pkt, (dst, args.udp)); s.close()
