...
def totp_now(secret: bytes, for_time: int, step=30, digits=6) -> int:
    ...
    digest = hmac.new(secret, msg, hashlib.sha1).digest(); off = digest[-1] & 0x0F
...
class Api(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/enroll":
            ...
            S.enrolled[kid]=raw; save_enrolled(); return self._json(200, {"ok":True})
        self._json(404, {"error":"not_found"})

def handle_packet(pkt: bytes, src_ip: str, open_ttl_default: int):
    try:
        if len(pkt) < 3+1+4+1+32+16+8+12+16: return
        if pkt[0:3] != b'PK6': return
        ver = pkt[3]
        if ver != 1: return
        off = 4
        ...
        cur = win_id()
        if win not in (cur, cur-1): return
        ...
        if bits < min(diff, S.difficulty_bits): return
...
def main():
    ...
    try:
        ...
        httpd.shutdown(); stop_sshd(); print("[CLEANUP] Bye.")
