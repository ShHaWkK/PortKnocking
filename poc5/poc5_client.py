...
- Traces HTTP en clair (console) + JSONL ~/.config/poc5/http_client.jsonl, avec CID commun.
"""
...
HTTP_LOG_PATH = os.path.join(STATE_DIR, "http_client.jsonl")
...
# --- traces HTTP (console + JSONL) avec CID
def http_log(event: str, **fields):
    try:
        os.makedirs(STATE_DIR, exist_ok=True)
...
def http_get_json(host: str, port: int, path: str, cid: str) -> tuple[int, dict]:
    conn = http.client.HTTPConnection(host, port, timeout=4.0)
    conn.putrequest("GET", path)
    conn.putheader("X-POC5-CID", cid)
    conn.endheaders()
    print(f"[HTTP-OUT] GET http://{host}:{port}{path} cid={cid}")
    http_log("http", side="client", dir="out", method="GET", url=f"http://{host}:{port}{path}", cid=cid)
    r = conn.getresponse(); data=r.read(); conn.close()
    body = data.decode(errors="ignore")
    print(f"[HTTP-IN ] {r.status} cid={cid} bytes={len(data)} body={body[:160]}")
    http_log("http", side="client", dir="in", status=r.status, cid=cid, bytes=len(data), body_preview=body[:160])
    try: return r.status, json.loads(body or "{}")
    except Exception: return r.status, {}

def http_post_json(host: str, port: int, path: str, obj: dict, cid: str) -> tuple[int, dict]:
    ...
    conn.endheaders()
    conn.send(body)
    print(f"[HTTP-OUT] POST http://{host}:{port}{path} cid={cid} bytes={len(body)} body={body[:160].decode('utf-8','ignore')}")
    http_log("http", side="client", dir="out", method="POST", url=f"http://{host}:{port}{path}", cid=cid,
             bytes=len(body), body_preview=body[:160].decode("utf-8","ignore"))
    r = conn.getresponse(); data=r.read(); conn.close()
    text = data.decode(errors="ignore")
    print(f"[HTTP-IN ] {r.status} cid={cid} bytes={len(data)} body={text[:160]}")
    http_log("http", side="client", dir="in", status=r.status, cid=cid, bytes=len(data), body_preview=text[:160])
    try: return r.status, json.loads(text or "{}")
    except Exception: return r.status, {}
...
def main():
    ...
    cid = os.urandom(4).hex()  # même CID pour /ports, /enroll, /knockqr
    print(f"Cible={dst_ip} | kid={kid} | cid={cid} | delay={args.delay}s | duration={args.duration}s")

    # ports actifs
    code, info = http_get_json(dst_ip, args.http_port, "/ports", cid)
    if code != 200:
    ...
    code, _ = http_post_json(dst_ip, args.http_port, "/enroll",
                             {"kid": kid, "pubkey": b64u(pk_raw)}, cid)
    if code != 200:
    ...
    code, resp = http_post_json(dst_ip, args.http_port, "/knockqr", {"qr": qr_b64}, cid)
    if code != 200:
