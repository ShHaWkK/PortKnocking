...
- Auto-install Python: numpy, scapy
- Auto-install système: nftables, openssh-server, (libpcap/qrencode selon distro)
- Démo locale: interface 'lo', port leurre 443, SPA UDP 45445, SSH 2222
"""

import os, sys, time, json, base64, hmac, hashlib, math, argparse, threading, subprocess
from typing import List, Tuple
# --- autoinstall ---
from autodeps import ensure_everything, have_bin

# ----------  Dépendances Python & Système ----------
PIP_PKGS = ["numpy>=1.20", "scapy"]
SYS_PKGS = {
    "apt":    ["nftables","openssh-server","qrencode","libpcap0.8","libpcap0.8-dev"],
...
if not ensure_everything(PIP_PKGS, SYS_PKGS):
    print("[!] Certaines dépendances n'ont pas pu être installées automatiquement.")
    print("    -> Lance le script install_deps.sh ou installe manuellement (messages ci-dessus).")
...
def run(*cmd, check=True, quiet=False):
    ...
    if not quiet and r.stdout.strip(): print(r.stdout.rstrip())
...
"""
    with open("/tmp/nft5.nft","w") as f: f.write(rules)
...
def fit_gaussians(preamble_delays:List[float], preamble_bits:str)->Tuple[Tuple[float,float],Tuple[float,float]]:
    ...
    mu0=float(np.median(zs0)); mu1=float(np.median(zs1))
    sig0=float(np.std(zs0) or 1e-3); sig1=float(np.std(zs1) or 1e-3)
    return (mu0,sig0), (mu1,sig1)

def logp_gauss(x, mu, sigma):  # log N(x|mu,sigma)
    return -0.5*math.log(2*math.pi*sigma*sigma) - ((x-mu)**2)/(2*sigma*sigma)

def viterbi_delays(delays:List[float], g0, g1)->str:
    n=len(delays); dp=np.full((n,2), -1e18); prev=np.zeros((n,2), dtype=int)
    ...
    for i in range(1,n):
        o0=logp_gauss(delays[i], *g0); o1=logp_gauss(delays[i], *g1)
        ps0=dp[i-1,0]+logA[0,0]; ps1=dp[i-1,1]+logA[1,0]
        if ps0>ps1: dp[i,0]=ps0+o0; prev[i,0]=0
        else:       dp[i,0]=ps1+o0; prev[i,0]=1
        ps0=dp[i-1,0]+logA[0,1]; ps1=dp[i-1,1]+logA[1,1]
        if ps0>ps1: dp[i,1]=ps0+o1; prev[i,1]=0
        else:       dp[i,1]=ps1+o1; prev[i,1]=1
    s=0 if dp[-1,0]>dp[-1,1] else 1
    states=[s]
    for i in range(n-1,0,-1):
        s=prev[i,s]; states.append(s)
    states=states[::-1]
    return "".join("1" if s==1 else "0" for s in states)
...
def start_sniffer(events, iface, stop_evt):
    def _cb(pkt):
        ...
        if int(pkt[TCP].dport)!=LURE_PORT: return
        if (pkt[TCP].flags & 0x02)==0: return  # SYN
...
def spa_listener(secret:bytes, stop_evt, on_open):
    ...
    s.bind(("0.0.0.0", SPA_PORT)); s.settimeout(0.2)
    while not stop_evt.is_set():
        try:
            data, addr = s.recvfrom(8192)
        except socket.timeout:
            continue
        if not data or data[0]!=0x01: continue
        blob=data[1:]
        if _hmac_ok(secret, blob):
            try:
                msg=json.loads(blob[:-16].decode())
                ip=addr[0]; dur=int(msg.get("duration", OPEN_TTL))
                on_open(ip, dur)
            except Exception:
                pass
    s.close()
...
def main():
    ...
    events=[]; stop_evt=threading.Event()
    ...
    try:
        ...
        while True:
            time.sleep(0.5)
            # Décode périodiquement chaque IP
            by_ip={}
            for ip,ts in events:
