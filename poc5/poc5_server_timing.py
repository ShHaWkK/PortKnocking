...
def ensure_pydeps():
    try: from cryptography.hazmat.primitives.ciphers.aead import AESGCM  # noqa
        ...
        LOG("BOOT", step="install_cryptography"); _pip("cryptography") or _apt("python3-cryptography") or _dnf("python3-cryptography") or _pacman("python-cryptography")
    try: import scapy.all  # noqa
    except Exception:
        LOG("BOOT", step="install_scapy"); _pip("scapy") or _apt("python3-scapy") or _dnf("python3-scapy") or _pacman("python-scapy")

def ensure_sysbins():
    missing=[]; 
    if not shutil.which("nft"): missing.append("nftables")
    if not os.path.exists("/usr/sbin/sshd"): missing.append("openssh-server")
    if missing: _apt(*missing) or _dnf(*missing) or _pacman(*missing)
...
# --- décode SPA (accepte 51 ou 53 octets)
def spa_parse_wire(wire: bytes, aes_key: bytes, hmac_key: bytes):
    try:
        ...
        nonce=wire[1:13]
        # fixed 51: ctag à partir de 13 ; legacy 53: longueur 2 octets à 13..15
...
def try_decode_for_ip(src_ip, deltas, aes_key, hmac_key):
    ...
    rough=_classify(deltas); idx=_find(rough, PREAMBULE)
    if idx<0:
        if len(deltas)>2000: deltas[:]=deltas[-1000:]
        return False

    # 1) tentative format FIXED (51 bytes → 416 bits)
    if len(rough) - idx >= BITS_FIXED:
        ...
        if spa:
            deltas.clear(); ttl=int(spa.get("duration", OPEN_TTL_S_DEFAULT)) or OPEN_TTL_S_DEFAULT
            nft_add_allowed(src_ip, ttl); return True

    # 2) tentative format LEGACY (preamble + 16 bits length + payload)
    ...
    # si rien, garde une fenêtre raisonnable
...
def loop_raw(aes_key, hmac_key):
    ...
    while not _stop:
        ...
        off=ihl; dport=int.from_bytes(pkt[off+2:off+4],"big"); flags=pkt[off+13]
        if dport!=LURE_PORT: continue
        if not (flags & 0x02) or (flags & 0x10): continue  # SYN sans ACK
        now=time.monotonic(); last=derniers.get(src_ip); derniers[src_ip]=now
        if last is None:
...
def cleanup():
    subprocess.run(["bash","-lc",f"nft list table inet {NFT_TABLE} >/dev/null 2>&1 && nft delete table inet {NFT_TABLE} || true"],check=False)
    stop_sshd(); LOG("CLEANUP"); jlog("server_stop")
