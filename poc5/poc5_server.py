...
# -*- coding: utf-8 -*-
"""
POC5 — Serveur (QR + traces HTTP)
- Knock: TCP SYN -> UDP -> ICMP (fenêtre N ou N-1, ports P1/P2 rotatifs via HMAC)
- API HTTP: GET /ports, POST /enroll, POST /knockqr (QR PNG base64)
- QR contient un SPA JSON signé Ed25519 (+ TOTP optionnel)
- Validation -> ajout IP dans nftables @allowed (timeout) -> SSH:2222
- Traces HTTP en clair (console) + JSONL /tmp/poc5_http_server.jsonl, avec CID commun.

Usage:
  sudo python3 poc5_server.py --iface lo
"""
...
HTTP_LOG_PATH      = "/tmp/poc5_http_server.jsonl" # JSONL côté serveur

# rate-limit API (token bucket simple)
RL_CAPACITY = 12.0
...
try:
    ...
    need("Installez: apt-get install -y libzbar0  puis  pip install pyzbar")
...
# --- trace HTTP util
def http_jsonl(event: str, **fields):
    try:
        rec = {"ts": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "event": event}
        rec.update(fields)
        with open(HTTP_LOG_PATH, "a") as f: f.write(json.dumps(rec, ensure_ascii=False)+"\n")
    except Exception:
        pass
...
class Api(BaseHTTPRequestHandler):
    ...
    def _json(self, code:int, obj:dict, cid:str, req_len:int, req_preview:str):
        raw = json.dumps(obj).encode()
        self.send_response(code)
        self.send_header("Content-Type","application/json")
        self.send_header("X-POC5-CID", cid)
        self.send_header("Content-Length", str(len(raw)))
        self.end_headers()
        self.wfile.write(raw)
        # traces
        print(f"[HTTP-SRV] {self.command} {self.path} cid={cid} -> {code} bytes_in={req_len} bytes_out={len(raw)} body_in={req_preview}")
        http_jsonl("http", side="server", method=self.command, path=self.path, cid=cid,
                   status=code, bytes_in=req_len, bytes_out=len(raw), body_in=req_preview)

    def _cid(self) -> str:
        return self.headers.get("X-POC5-CID","no-cid")
    ...
    def do_GET(self):
        ...
        if self.path.split("?")[0] == "/ports":
            ...
            body = {"p1":cur["p1"],"p2":cur["p2"],"expires":cur["exp"],
                    "rotate":S.rotate_s,"ssh":SSH_PORT,"http":S.http_port,
                    "totp_required": bool(S.totp_secret is not None)}
            self._json(200, body, cid, 0, "")
            return
        if self.path == "/healthz":
            self._json(200, {"ok":True}, cid, 0, "")
            return
        self._json(404, {"error":"not_found"}, cid, 0, "")

    def do_POST(self):
        cid = self._cid()
        ip  = self.client_address[0]
        if not rl_ok(ip):
            self._json(429, {"error":"rate_limited"}, cid, 0, "")
            return
        ln = int(self.headers.get("Content-Length","0") or "0")
        raw_in = self.rfile.read(ln) if ln>0 else b"{}"
        req_preview = raw_in[:160].decode("utf-8","ignore")
        try:
            data = json.loads(raw_in.decode()) if ln>0 else {}
        except Exception:
            self._json(400, {"error":"bad_json"}, cid, ln, req_preview); return

        if self.path == "/enroll":
            kid=str(data.get("kid",""))[:32]; pub=str(data.get("pubkey",""))
            if not kid or not pub:
                self._json(400, {"error":"missing_fields"}, cid, ln, req_preview); return
            try:
                raw=de_b64u(pub); Ed25519PublicKey.from_public_bytes(raw)
            except Exception:
                self._json(400, {"error":"bad_pubkey"}, cid, ln, req_preview); return
            S.enrolled[kid]=raw; save_enrolled()
            LOG("ENROLL_OK", ip=ip, kid=kid)
            self._json(200, {"ok":True}, cid, ln, req_preview); return

        if self.path == "/knockqr":
            if ip not in S.pending or S.pending[ip] < time.monotonic():
                self._json(403, {"error":"not_pending"}, cid, ln, req_preview); return

            png_b64 = str(data.get("qr","") or data.get("qr_b64",""))
            if not png_b64:
                self._json(400, {"error":"missing_qr"}, cid, ln, req_preview); return
            try:
                text = qr_text_from_png_b64(png_b64)
                spa  = json.loads(text)
            except Exception:
                self._json(400, {"error":"bad_qr"}, cid, ln, req_preview); return
            ...
            if not nonce or key in S.nonces:
                self._json(403, {"error":"replay_nonce"}, cid, ln, req_preview); return
            S.nonces[key] = nowm + 300.0

            # vérif signature
            pk_raw = S.enrolled.get(kid)
            if not pk_raw:
                self._json(403, {"error":"unknown_kid"}, cid, ln, req_preview); return
            pk = Ed25519PublicKey.from_public_bytes(pk_raw)
            payload = {k:v for k,v in spa.items() if k!="sig"}
            try:
                pk.verify(base64.urlsafe_b64decode(sig_b64 + "=" * (-len(sig_b64)%4)), canon_bytes(payload))
            except Exception:
                self._json(403, {"error":"bad_sig"}, cid, ln, req_preview); return

            # fraicheur + TOTP
            ts = int(spa.get("ts",0))
            if ts<=0 or abs(int(time.time())-ts)>90:
                self._json(403, {"error":"stale_ts"}, cid, ln, req_preview); return
            if S.totp_secret is not None:
                try: code=int(str(spa.get("totp","")).strip())
                except Exception:
                    self._json(403, {"error":"totp_required"}, cid, ln, req_preview); return
                if not totp_verify(S.totp_secret, code, int(time.time())):
                    self._json(403, {"error":"totp_bad"}, cid, ln, req_preview); return

            ttl = max(5, min(3600, int(spa.get("duration", S.open_ttl))))
            nft_open(ip, ttl)
            S.pending.pop(ip, None)
            self._json(200, {"ok":True, "ttl":ttl}, cid, ln, req_preview); return

        self._json(404, {"error":"not_found"}, cid, ln, req_preview)
